...
namespace storm::core {
    ...
    struct RGBColor {
        ...
        constexpr RGBColor(std::uint32_t argb) noexcept : a(255), g(0), b(0), r(0) {
            r = argb & 0xFF;
            g = (argb >> 8) & 0xFF;
            b = (argb >> 16) & 0xFF;
            a = (argb >> 24) & 0xFF;
        ...
        constexpr operator std::uint32_t() const noexcept {
            return ((std::uint32_t(a) << 24) & 0xFF000000) |
                   ...
                   ((std::uint32_t(g) << 8) & 0x0000FF00) | (std::uint32_t(b) & 0x000000FF);
        }
...
#ifdef STORM_GLM_CONVERSION
        template<
          typename T = glm::uvec3,
          typename =
            std::enable_if_t<std::is_same_v<std::decay_t<std::remove_cv_t<T>>, glm::uvec3>>>
        static RGBColor fromVec3(T &&vec) {
            return RGBColor(vec.x, vec.y, vec.z);
        }

        template<
          typename T = glm::uvec4,
          typename =
            std::enable_if_t<std::is_same_v<std::decay_t<std::remove_cv_t<T>>, glm::uvec4>>>
    ...
    struct RGBColorF {
        ...
        template<
          typename T = RGBColor,
          typename = std::enable_if_t<std::is_same_v<std::decay_t<std::remove_cv_t<T>>, RGBColor>>>
        ...
        constexpr inline bool operator==(const RGBColor &color) const noexcept {
            ...
            return realIsEqual(r, _r) && realIsEqual(g, _g) && realIsEqual(b, _b) &&
                   realIsEqual(a, _a);
        }

        constexpr inline bool operator==(const RGBColorF &color) const noexcept {
            return realIsEqual(r, color.r) && realIsEqual(g, color.g) && realIsEqual(b, color.b) &&
                   realIsEqual(a, color.a);
        }
        ...
        template<
          typename T = glm::vec3,
          typename = std::enable_if_t<std::is_same_v<std::decay_t<std::remove_cv_t<T>>, glm::vec3>>>
        ...
        template<
          typename T = glm::vec4,
          typename = std::enable_if_t<std::is_same_v<std::decay_t<std::remove_cv_t<T>>, glm::vec4>>>
