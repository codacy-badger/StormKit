...
namespace storm::core {
    ...
    constexpr inline Result ComponentHash(const char *str, std::size_t size) {
        return size == 0
                 ...
                 : (static_cast<std::size_t>(str[ 0 ]) ^ ComponentHash<Result>(str + 1, size - 1)) *
                     0x100000001b3UL;
    }
    ...
    constexpr inline std::uint64_t operator"" _hash(const char *str, std::size_t size) {
...
namespace std {
    ...
    struct hash<std::vector<T>> {
        std::size_t operator()(const std::vector<T> &in) const {
            ...
            std::size_t seed = 0;
            for(size_t i = 0; i < size; i++) storm::core::hash_combine(seed, in[ i ]);
