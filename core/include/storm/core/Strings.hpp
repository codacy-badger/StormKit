...
namespace storm::core {
	...
	template <typename T>
	using ostream_expression = decltype(std::declval<std::ostringstream&>() << std::declval<T>().toString());
	...
	namespace _private {
		...
		std::string populate(const std::string &string, const size_t index, T &&value, Args&&... args) {
			...
			if constexpr(!sizeof...(args))
				...
				return populate(std::regex_replace(string, regex, value_string), index + 1, std::forward<Args>(args)...);
		}
	}
