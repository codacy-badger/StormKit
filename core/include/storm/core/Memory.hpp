...
namespace storm::core {
    ...
    template<typename T>
    constexpr inline std::remove_cv_t<std::remove_reference_t<T>>
    getBits(const T &v, std::size_t offset, std::size_t size) noexcept {
        std::remove_const_t<std::remove_reference_t<T>> b = 0;

        for(std::uint_fast32_t i = 0; i < size; ++i) b or_eq (getBit(v, offset + i) << i);
    ...
    constexpr inline void setBits(T &t, V &&v, std::size_t pos, std::size_t size) noexcept {
        for(std::uint_fast32_t i = 0; i < size; ++i) setBit(t, (v >> i) & 1, pos + i);
    }
