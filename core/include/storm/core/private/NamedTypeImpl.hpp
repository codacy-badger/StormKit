...
namespace storm::core {
	...
	class STORM_EBCO NamedType : public NamedTypeTrait, public Skills<NamedType<T, Parameter, Skills...>>...
	{
	public:
		using UnderlyingType = T;

		template <typename... Args>
		explicit constexpr NamedType(Args&& ...args) : value_(std::forward<Args>(args)...) {}
	...
	constexpr StrongType makeNamed(Args&& ...args) {
		return StrongType(typename StrongType::UnderlyingType(std::forward<Args>(args)...));
	}
}
