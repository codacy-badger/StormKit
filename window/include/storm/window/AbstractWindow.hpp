...
namespace storm::window {
	...
	public:
		...
		virtual void create(const std::string &title,
		    const VideoSettings &settings, WindowStyle style)
		    = 0;
		...
		virtual bool pollEvent(
		    Event &event, void *native_event = nullptr) noexcept;
		virtual bool waitEvent(
		    Event &event, void *native_event = nullptr) noexcept
		    = 0;

		virtual void setTitle(const std::string &title) noexcept = 0;
		virtual void setVideoSettings(const VideoSettings &settings) noexcept
		    = 0;
		...
		virtual void setMousePosition(std::int16_t x, std::int16_t y) const
		    noexcept
		    = 0;
		...
		void mouseDownEvent(
		    MouseButton button, std::int16_t x, std::int16_t y) noexcept;
		void mouseUpEvent(
		    MouseButton button, std::int16_t x, std::int16_t y) noexcept;
