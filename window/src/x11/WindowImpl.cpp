...
void WindowImpl::create(const std::string &title, const VideoSettings &settings, WindowStyle style) {
    ...
    std::uint32_t value_list[] = {screen->black_pixel, screen->black_pixel, EVENTS};
	xcb_create_window(m_connection.get(),
					  XCB_COPY_FROM_PARENT,
					  m_window, screen->root,
					  0, 0, settings.size.w, settings.size.h,
					  0,
					  XCB_WINDOW_CLASS_INPUT_OUTPUT,
					  screen->root_visual,
                     XCB_CW_BACK_PIXEL | XCB_CW_BORDER_PIXEL | XCB_CW_EVENT_MASK, value_list);
	...
	if((style & WindowStyle::Fullscreen) == WindowStyle::Fullscreen) {
		...
		const auto atom_request = xcb_intern_atom(m_connection.get(), 0, window_manager_hints_str.length(), window_manager_hints_str.data());
		...
		if(atom_reply) {
			...
			//applying hints
			xcb_change_property(m_connection.get(), XCB_PROP_MODE_REPLACE, m_window, atom_reply->atom, XCB_ATOM_WM_HINTS, 32, 5, reinterpret_cast<std::byte*>(&hints));
...
void WindowImpl::setTitle(const std::string &title) noexcept {
	xcb_change_property(m_connection.get(), XCB_PROP_MODE_REPLACE, m_window,
						XCB_ATOM_WM_NAME, XCB_ATOM_STRING, 8, static_cast<std::uint32_t>(title.length()), title.c_str());
}
...
VideoSettings::Size WindowImpl::size() const noexcept {
	const auto reply = xcb_get_geometry_reply(m_connection.get(), xcb_get_geometry(m_connection.get(), m_window), nullptr);
...
void WindowImpl::processEvents(xcb_generic_event_t xevent, void *native_event) {
	...
	switch(xevent.response_type & ~0x80) {
		case XCB_KEY_PRESS: {
			...
			AbstractWindow::mouseDownEvent(x11MouseButtonToStormMouseButton(button), button_event->event_x, button_event->event_y);
			break;
		} case XCB_BUTTON_RELEASE: {
			auto button_event = reinterpret_cast<xcb_button_press_event_t*>(&xevent);

			auto button = button_event->detail;
			AbstractWindow::mouseUpEvent(x11MouseButtonToStormMouseButton(button), button_event->event_x, button_event->event_y);
			break;
		...
		case XCB_VISIBILITY_NOTIFY:
			/*if(xevent.xvisibility.window == m_window)
				m_is_visible = xevent.xvisibility.state != VisibilityFullyObscured;*/
			break;
