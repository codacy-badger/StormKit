...
void WindowImpl::create(
  ...
  WindowStyle style) {
    ...
    if((style & WindowStyle::Fullscreen) == WindowStyle::Fullscreen) {
        ...
        const auto atom_request = xcb_intern_atom(
          m_connection.get(),
          0,
...
void WindowImpl::setTitle(const std::string &title) noexcept {
    xcb_change_property(
      ...
      XCB_ATOM_STRING,
      8,
      static_cast<std::uint32_t>(title.length()),
      title.c_str());
...
VideoSettings::Size WindowImpl::size() const noexcept {
    const auto reply = xcb_get_geometry_reply(
      m_connection.get(), xcb_get_geometry(m_connection.get(), m_window), nullptr);
...
void WindowImpl::processEvents(xcb_generic_event_t xevent, void *native_event) {
    ...
    switch(xevent.response_type & ~0x80) {
        ...
        case XCB_BUTTON_PRESS: {
            ...
            AbstractWindow::mouseDownEvent(
              x11MouseButtonToStormMouseButton(button),
