...
WindowImpl::WindowImpl(const std::string &title, const VideoSettings &settings,
    WindowStyle style, const ContextSettings &context_settings) noexcept {
...
void WindowImpl::create(const std::string &title, const VideoSettings &settings,
	...
	m_window.create({settings.size.w, settings.size.h, settings.bpp}, title,
	    _style, _settings);
}

void WindowImpl::close() noexcept { m_window.close(); }

void WindowImpl::display() noexcept { m_window.display(); }

bool WindowImpl::pollEvent(Event &event) noexcept { return false; }
...
bool WindowImpl::isOpen() const noexcept { return m_window.isOpen(); }

bool WindowImpl::isVisible() const noexcept { return true; }
