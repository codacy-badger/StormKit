...
namespace storm::image {
	...
	class STORM_PUBLIC Image {
		public:
			...
			enum class Codec {
				AUTODETECT, JPEG, PNG, TARGA, PPM, UNKNOW
			};
			enum class CodecArgs {
				BINARY, ASCII // for ppm
			};
			...
			explicit Image();
			explicit Image(const _std::filesystem::path &filepath, Codec codec = Codec::AUTODETECT);
			explicit Image(std::uint8_t *data, size_type size, Codec codec = Codec::AUTODETECT);
			~Image() = default;

			void loadFromFile(const _std::filesystem::path &filepath, Codec codec = Codec::AUTODETECT);
			void loadFromMemory(std::uint8_t *data, size_type size, Codec codec = Codec::AUTODETECT);
			void create(std::uint32_t width, std::uint32_t height, std::uint32_t channel_count = 3);
			...
			void saveToFile(const _std::filesystem::path &filename, Codec codec = Codec::AUTODETECT, CodecArgs args = CodecArgs::BINARY);
