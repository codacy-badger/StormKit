...
namespace storm::image {
    Image::Codec filenameToCodec(const _std::filesystem::path &filename) {
        ...
        if(storm::core::to_lower(ext) == ".jpg" || storm::core::to_lower(ext) == ".jpeg")
            return Image::Codec::JPEG;
        else if(storm::core::to_lower(ext) == ".png")
            return Image::Codec::PNG;
        else if(storm::core::to_lower(ext) == ".tga" || storm::core::to_lower(ext) == ".targa")
...
/////////////////////////////////////
/////////////////////////////////////
Image::Image(const _std::filesystem::path &filepath, Image::Codec codec) : Image() {
...
/////////////////////////////////////
/////////////////////////////////////
void Image::loadFromFile(const _std::filesystem::path &filepath, Image::Codec codec) {
    detach();
...
/////////////////////////////////////
/////////////////////////////////////
void Image::loadFromMemory(std::uint8_t *data, size_type size, Image::Codec codec) {
    detach();

    ASSERT(codec != Image::Codec::AUTODETECT, "Autodetection only for file based image loading");
    ASSERT(codec != Image::Codec::UNKNOW, "Unknow image filetype");

    switch(codec) {
        case Image::Codec::JPEG: m_data = private_::ImageLoader::loadJpeg(data, size); break;
        case Image::Codec::PNG: m_data = private_::ImageLoader::loadPng(data, size); break;
        case Image::Codec::TARGA: m_data = private_::ImageLoader::loadTga(data, size); break;
        case Image::Codec::PPM: m_data = private_::ImageLoader::loadPPM(data, size); break;
        default: throw std::runtime_error("oops");
    }
}
...
/////////////////////////////////////
/////////////////////////////////////
void Image::create(std::uint32_t width, std::uint32_t height, std::uint32_t channel_count) {
...
std::uint8_t *Image::operator()(Image::size_type x, Image::size_type y) {
    ...
    std::uint8_t *pointer = &m_data->data[ (y * m_data->size.width + x) * m_data->channel ];
...
/////////////////////////////////////
/////////////////////////////////////
const std::uint8_t *Image::operator()(Image::size_type x, Image::size_type y) const noexcept {
    const std::uint8_t *pointer = &m_data->data[ (y * m_data->size.width + x) * m_data->channel ];
...
/////////////////////////////////////
/////////////////////////////////////
Image Image::scale(const Image &src, const Image::Size &newSize) {
    return src;
}
        ...
        for(auto y = 0; y < out.size().height; ++y)
            for(auto k = 0; k < src.channels(); ++k)
                out.data()[ (x + y * out.size().width) * src.channels() + k ] =
                  src.data()
                    [ (x + (out.size().height) * out.size().width - 1 - x) * src.channels() + k ];
    ...
    for(auto x = 0; x < out.size().width; ++x)
        for(auto y = 0; y < out.size().height; ++y)
            for(auto k = 0; k < src.channels(); ++k)
                out.data()[ (x + y * out.size().width) * src.channels() + k ] =
                  src.data()
                    [ (x + (out.size().height - 1 - y) * out.size().width) * src.channels() + k ];
...
/////////////////////////////////////
/////////////////////////////////////
void Image::saveToFile(const _std::filesystem::path &filename, Codec codec, CodecArgs args) {
...
void Image::addChannels(std::uint8_t count) {
    ...
    for(auto i = 0; i < pixel_count; ++i) {
        std::copy_n(
          std::begin(m_data->data) + (i * m_data->channel),
          m_data->channel,
          std::back_inserter(image_data));

        for(auto j = 0; j < count; ++j) image_data.emplace_back(0);
    }
