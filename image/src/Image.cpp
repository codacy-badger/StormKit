	...
	for (auto x = 0; x < out.size().width; ++x)
		for (auto y = 0; y < out.size().height; ++y)
			for(auto k = 0;k < src.channels(); ++k)
				out.data()[(x + y * out.size().width) * src.channels() + k] = src.data()[(x + (out.size().height) * out.size().width - 1 - x) * src.channels() + k];
...
Image Image::flipY(const Image &src) {
	...
	for (auto x = 0; x < out.size().width; ++x)
		for (auto y = 0; y < out.size().height; ++y)
			for(auto k = 0;k < src.channels(); ++k)
				out.data()[(x + y * out.size().width) * src.channels() + k] = src.data()[(x + (out.size().height - 1 - y) * out.size().width) * src.channels() + k];
...
void Image::addChannels(std::uint8_t count) {
	...
	for(auto i = 0;i < pixel_count; ++i) {
		std::copy_n(std::begin(m_data->data) + (i * m_data->channel), m_data->channel, std::back_inserter(image_data));
