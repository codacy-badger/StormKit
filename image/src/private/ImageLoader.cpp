...

    {
        ...
        if(!file)
            throw std::runtime_error(
              storm::core::format("Failed to open file %{1}", filename.string()));

        unsigned char header[ 8 ];
        fread(header, 1, 8, file);
        if(png_sig_cmp(header, 0, 8))
            throw std::runtime_error(
              storm::core::format("[libpng] Failed to read header of %{1}", filename.string()));

        auto png_ptr = png_create_read_struct(PNG_LIBPNG_VER_STRING, nullptr, nullptr, nullptr);
        if(!png_ptr)
            throw std::runtime_error(storm::core::format(
              "[libpng] Failed to init (png_create_read_struct) %{1}", filename.string()));

        auto info_ptr = png_create_info_struct(png_ptr);
        if(!info_ptr)
            throw std::runtime_error(storm::core::format(
              "[libpng] Failed to init (png_create_info_struct) %f", filename.string()));

        if(setjmp(png_jmpbuf(png_ptr)))
            throw std::runtime_error(
              storm::core::format("[libpng] Failed to init io %{1}", filename.string()));
        ...
        if(setjmp(png_jmpbuf(png_ptr)))
            throw std::runtime_error(
              storm::core::format("[libpng] Failed to read image %{1}", filename.string()));
        ...
        for(size_t i = 0; i < data->size.height; ++i)
            row_pointers[ i ] = reinterpret_cast<png_bytep>(
              buff_pos + ((data->size.height - (i + 1)) * data->size.width * data->channel));
...
ImageData::SPtr ImageLoader::loadJpeg(const _std::filesystem::path &filename) {
...

    {
        ...
        while(info.output_scanline < info.output_height) {
            rowptr[ 0 ] =
              data->data.data() + data->size.width * data->channel * info.output_scanline;
...
ImageData::SPtr ImageLoader::loadJpeg(std::uint8_t *data, Image::size_type size) {
...

    {
        ...
        while(info.output_scanline < info.output_height) {
            rowptr[ 0 ] =
              data_->data.data() + data_->size.width * data_->channel * info.output_scanline;
            jpeg_read_scanlines(&info, rowptr, 1);
        }
...
ImageData::SPtr ImageLoader::loadTga(const _std::filesystem::path &filename) {
...

    {
        ...
        data->channel = 3;
        if(header.header[ 4 ] > 24) data->channel = 4;
            ...
            do {
                ...
                if(chunk_header < 128) {
                    ...
                    for(auto i = 0; i < chunk_header; ++i) {
                        ...
                        if(bytes == 4) data->data[ current_byte + 3 ] = colorbuffer[ 3 ];
