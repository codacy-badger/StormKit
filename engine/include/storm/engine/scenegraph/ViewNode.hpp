...
namespace storm::engine {
	class STORM_PUBLIC ViewNode : public Node {
		public:
			...
			template <typename T = mat4, typename = std::enable_if_t<std::is_same_v<std::decay_t<std::remove_cv_t<T>>, mat4>>>
			inline void setViewMatrix(T&& view) noexcept { m_view_matrix = std::forward<T>(view); commit(); }
