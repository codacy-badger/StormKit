...
namespace storm::engine {
	...
	ResourceBase::ID RenderTaskBuilder::create(std::string name,
	    Description &&                                     description,
	    std::vector<std::uint32_t> &                       create_resources) {
		auto id = 0u;
		if constexpr (std::is_same_v<std::decay_t<Resource>, TextureResource>)
			id = m_pool.addTransientTexture(m_device, std::move(name),
			    std::forward<Description>(description), std::ref(m_task));
		else if constexpr (std::is_same_v<std::decay_t<Resource>,
		                       UniformBufferResource>)
			id = m_pool.addTransientUniformBuffer(m_device, std::move(name),
			    std::forward<Description>(description), std::ref(m_task));
		else
			static_assert(true, "Invalid Resource type");
	...
	ResourceBase::ID RenderTaskBuilder::write(ResourceBase::ID resourceID,
	    std::vector<std::uint32_t> &write_resources) {
	...
	ResourceBase::ID RenderTaskBuilder::read(ResourceBase::ID resourceID,
	    std::vector<std::uint32_t> &                          read_resources) {
		auto &resource = m_pool.getResource(resourceID);
