...
namespace storm::engine {
	...
	public:
		...
		ResourceBase::ID createAttachment(
		...
		ResourceBase::ID createUniformBuffer(
	...
	private:
		...
		ResourceBase::ID create(std::string name, Description &&description,
		    std::vector<std::uint32_t> &create_resources);

		template <typename Resource>
		ResourceBase::ID write(ResourceBase::ID attachment,
		    std::vector<std::uint32_t> &        write_resources);

		template <typename Resource>
		ResourceBase::ID read(ResourceBase::ID attachment,
