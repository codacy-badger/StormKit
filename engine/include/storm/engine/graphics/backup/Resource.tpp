...
namespace storm::engine {
	...
	) : ResourceBase{std::move(name), std::move(creator), core::ComponentHash<std::uint64_t>(typeid(Resource<ResourceDescription_, ResourceType_>).name())},
	...
	) : ResourceBase{std::move(name), std::nullopt, core::ComponentHash<std::uint64_t>(typeid(Resource<ResourceDescription_, ResourceType_>).name())},
		m_description{std::forward<ResourceDescription__>(description)} {
		m_resource = &resource;
	}
