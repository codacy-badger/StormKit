...
namespace storm::engine {
	...
	constexpr inline RenderMethod operator"" _hash(
	...
	public:
		...
		using GraphUpdateFunction
		    = std::function<void(RenderGraph &, const GraphUpdateParameters &)>;
		...
		inline void setProjectionMatrix(mat4 projection) {
			m_camera.projection       = std::move(projection);
			m_camera_dirty.projection = true;
		}
		inline void setViewMatrix(mat4 view) {
			m_camera.view       = std::move(view);
			m_camera_dirty.view = true;
		}

		inline const Mesh &getMesh(RenderMethod method, MeshID id) const
		    noexcept {
			ASSERT(id < std::size(m_meshs.at(method)), "Out of bound");
			return m_meshs.at(method)[id];
		}
		inline const PipelineState &basePipelineState() const noexcept {
			return m_base_pipeline_state;
		}
	...
	private:
		...
		std::unordered_map<RenderMethod, GraphUpdateFunction>
		    m_graph_update_funcs;
