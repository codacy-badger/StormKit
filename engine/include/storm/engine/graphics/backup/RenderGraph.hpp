...
namespace storm::engine {
	class RenderGraph : public core::NonCopyable {
		...
		private:
			struct Step {
				Step(RenderTaskBase::ID task_, std::vector<ResourceBase::ID> &&realized_resources_, std::vector<ResourceBase::ID> &&derealized_resources_)
					: task{task_}, realized_resources{realized_resources_}, derealized_resources{derealized_resources_} {}
