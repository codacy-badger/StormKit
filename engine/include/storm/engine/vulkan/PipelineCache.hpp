...
namespace std {
	...
	struct hash<storm::engine::PipelineCacheKey> {
		std::size_t operator()(const storm::engine::PipelineCacheKey &key) const {
			...
			storm::core::hash_combine(hash, std::hash<const storm::engine::RenderPass::SubPass*>{}(key.subpass));
			storm::core::hash_combine(hash, std::hash<const storm::engine::Program*>{}(key.program));
