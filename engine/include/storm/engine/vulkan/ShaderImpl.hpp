...
namespace storm::engine {
	class ShaderImpl {
		public:
			...
			explicit ShaderImpl(const Device &device, Shader::Stage stage, const _std::filesystem::path &path);
			explicit ShaderImpl(const Device &device, Shader::Stage stage, const std::byte* data, std::size_t size);
			~ShaderImpl();
