...
namespace storm::engine {
	...
	class CommandBufferImpl {
		public:
			...
			void beginRenderPass(RenderPass &render_pass, core::RGBColorF clear_color);
			void endRenderPass();
			...
			void submit(const std::vector<const Semaphore*> &wait_semaphores, const std::vector<const Semaphore*> &signal_semaphores,
			...
			void blitImage(vk::Image source, vk::Image destination, const glm::uvec2 &src_size,
						   const glm::uvec2 &destination_size, vk::Filter filter) const;
			void blitImage(vk::Image source, uint32_t source_mipmap_level, vk::Image destination,
						   uint32_t destination_mipmap_level, const glm::uvec2 &src_size, const glm::uvec2 &destination_size,
						   uint32_t layer_count, vk::Filter filter) const;
			void copyBuffer(const vk::Buffer &src, const vk::Buffer &dest, vk::DeviceSize size) const;
			void copyBufferToImage(const vk::Buffer &src, const vk::Image &dest, std::uint32_t width, std::uint32_t height) const;
