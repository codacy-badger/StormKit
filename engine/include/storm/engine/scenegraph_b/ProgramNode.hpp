...
namespace storm::engine {
	...
	public:
		...
		inline const HoldedType &programID() const noexcept {
			return m_program_id;
		}

		template <typename T = HoldedType,
		    typename         = std::enable_if_t<
                std::is_same_v<std::decay_t<std::remove_cv_t<T>>,
                    HoldedType> || std::is_constructible_v<HoldedType, T>>>
		inline void setProgramID(T &&id) noexcept {
			m_program_id = std::forward<T>(id);
