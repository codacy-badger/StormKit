...
namespace storm::engine {
	...
	inline std::size_t VertexBuffer::addData(Container &&container) {
		return addData(
	...
	template <class Container>
	inline void VertexBuffer::updateData(
	    Container &&container, std::ptrdiff_t offset) {
		updateData(reinterpret_cast<const std::byte *>(std::data(container)),
		    std::size(container)
		        * sizeof(
		            typename std::remove_reference_t<Container>::value_type),
		    offset);
