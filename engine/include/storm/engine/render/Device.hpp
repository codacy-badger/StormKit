...
namespace storm::engine {
    ...
    public:
        ...
        Shader::Ptr
        createShaderPtr(Shader::Stage stage, const std::byte *data, std::size_t size) const;

        RenderPass
        createRenderPass(bool default_subpass = false, bool enable_depth_attachment = false) const;
        RenderPass::Ptr createRenderPassPtr(
          bool default_subpass         = false,
          bool enable_depth_attachment = false) const;

        Framebuffer createFramebuffer() const;
        Framebuffer::Ptr createFramebufferPtr() const;

        HardwareBuffer createHardwareBuffer(HardwareBuffer::Description description) const;
        HardwareBuffer::Ptr createHardwareBufferPtr(HardwareBuffer::Description description) const;
        ...
        Texture
        createTexture(Texture::Description description, const storm::image::Image &image) const;
        Texture createTexture(Texture::Description description) const;
        Texture::Ptr
        createTexturePtr(Texture::Description description, const storm::image::Image &image) const;
