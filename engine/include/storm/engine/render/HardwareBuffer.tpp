...
namespace storm::engine {
    ...
    inline void HardwareBuffer::setData(Container &&container, std::ptrdiff_t offset) {
        using value_type = typename std::remove_reference_t<Container>::value_type;

        const auto container_size  = std::size(container);
        const auto container_data  = reinterpret_cast<const std::byte *>(std::data(container));
