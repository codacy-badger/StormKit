...
void runApp() {
	...
	auto vertex_shader = device.createShader(engine::Shader::Stage::VERTEX, "shaders/uniformbuffer_spv.vert");
	auto fragment_shader = device.createShader(engine::Shader::Stage::FRAGMENT, "shaders/uniformbuffer_spv.frag");
	...
	matrices.view = glm::lookAt(glm::vec3{2.f, 2.f, 2.f},
								{0.f, 0.f, 0.f},
								{0.f, 0.f, 1.f});
	...
	while(render_window.isOpen()) {
		...
		matrices.model = glm::rotate(glm::mat4{1.f}, time * glm::radians(90.0f), glm::vec3{0.0f, 0.0f, 1.0f});
		uniform_buffer.updateData(reinterpret_cast<std::byte*>(&matrices.model), sizeof(glm::mat4), offsetof(Matrices, model));
