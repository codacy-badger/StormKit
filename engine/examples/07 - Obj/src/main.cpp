...
void runApp() {
    ...
    auto render_window =
      window::Window{"StormKit obj loading Example",
    ...
    auto indices  = IndexArray{};
    {
        ...
        if(!tinyobj::LoadObj(&attribs, &shapes, &materials, &warn, &err, "models/chalet.obj")) {
    ...
    auto vertex_shader = device.createShader(engine::Shader::Stage::VERTEX, "shaders/obj_spv.vert");
    auto fragment_shader =
      device.createShader(engine::Shader::Stage::FRAGMENT, "shaders/obj_spv.frag");
    ...
    const auto index_buffer_desc =
      engine::HardwareBuffer::Description{std::size(indices) * sizeof(std::uint32_t),
    ...
    while(render_window.isOpen()) {
        ...
        if(std::chrono::duration_cast<std::chrono::seconds>(now - last_fps_update).count() >= 1.f) {
            ...
            render_window.setTitle(core::format(
              "StormKit obj loading Example | Delta : %{1} | FPS : %{2}", delta, 1.f / delta));
            last_fps_update = now;
        }
