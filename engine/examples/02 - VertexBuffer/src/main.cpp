...
void runApp() {
    ...
    auto render_window =
      window::Window{"StormKit Vertex buffer Example",
    ...
    auto vertex_shader =
      device.createShader(engine::Shader::Stage::VERTEX, "shaders/vertexbuffer_spv.vert");
    auto fragment_shader =
      device.createShader(engine::Shader::Stage::FRAGMENT, "shaders/vertexbuffer_spv.frag");
    ...
    while(render_window.isOpen()) {
        ...
        if(std::chrono::duration_cast<std::chrono::seconds>(now - last_fps_update).count() >= 1.f) {
            ...
            render_window.setTitle(core::format(
              "StormKit Vertex Buffer Example | Delta : %{1} | FPS : %{2}", delta, 1.f / delta));
            last_fps_update = now;
        }
