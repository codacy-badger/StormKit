...
void runApp() {
    ...
    auto render_window =
      window::Window{"StormKit Texture Example",
    ...
    auto vertex_shader =
      device.createShader(engine::Shader::Stage::VERTEX, "shaders/texture_spv.vert");
    auto fragment_shader =
      device.createShader(engine::Shader::Stage::FRAGMENT, "shaders/texture_spv.frag");
    ...
    matrices.model      = glm::translate(
      glm::mat4{1.f}, {WINDOW_WIDTH<float> / 2.f - 150.f, WINDOW_HEIGHT<float> / 2.f - 189.f, 0.f});
    ...
    while(render_window.isOpen()) {
        ...
        if(std::chrono::duration_cast<std::chrono::seconds>(now - last_fps_update).count() >= 1.f) {
            ...
            render_window.setTitle(core::format(
              "StormKit Texture Example | Delta : %{1} | FPS : %{2}", delta, 1.f / delta));
            last_fps_update = now;
        }
