...
static const auto INDICES = std::array<std::uint16_t, 12>{0, 1, 2, 3, 1, 0, 4, 5, 6, 7, 5, 4};
...
void runApp() {
    ...
    auto render_window =
      window::Window{"StormKit Depth buffer Example",
    ...
    auto vertex_shader =
      device.createShader(engine::Shader::Stage::VERTEX, "shaders/depthbuffer_spv.vert");
    auto fragment_shader =
      device.createShader(engine::Shader::Stage::FRAGMENT, "shaders/depthbuffer_spv.frag");
    ...
    const auto index_buffer_desc =
      engine::HardwareBuffer::Description{std::size(INDICES) * sizeof(std::uint16_t),
                                          alignof(std::uint16_t),
                                          engine::BufferUsageFlag::INDEX};

    auto index_buffer = device.createHardwareBuffer(std::move(index_buffer_desc));
    ...
    while(render_window.isOpen()) {
        ...
        if(std::chrono::duration_cast<std::chrono::seconds>(now - last_fps_update).count() >= 1.f) {
            auto delta =
              std::chrono::duration<float, std::chrono::seconds::period>{now - last}.count();
            if(delta == 0) delta = 1;
            render_window.setTitle(core::format(
              "StormKit Depth Buffer Example | Delta : %{1} | FPS : %{2}", delta, 1.f / delta));
            last_fps_update = now;
        }
