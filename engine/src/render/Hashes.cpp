...
static auto color_component_hasher    = std::hash<ColorComponentFlag>{};
...
namespace std {
    std::size_t hash<PipelineState>::operator()(const PipelineState &pipeline) const {
        using namespace storm::core;
        using namespace storm::engine;

        auto hash = bool_hasher(pipeline.is_blending_enabled);
        hash_combine(hash, color_component_hasher(pipeline.blend_write_color_mask));
        ...
        hash_combine(hash, vertex_input_state_hasher(pipeline.vertex_input_state));
    ...
    bool equal_to<PipelineState>::
    operator()(const PipelineState &first, const PipelineState &second) const noexcept {
        return pipeline_state_hasher(first) == pipeline_state_hasher(second);
    }

    std::size_t hash<VertexInputState>::operator()(const VertexInputState &obj) const {
    ...
    bool equal_to<VertexInputState>::
    operator()(const VertexInputState &first, const VertexInputState &second) const noexcept {
        return vertex_input_state_hasher(first) == vertex_input_state_hasher(second);
    }

    std::size_t hash<VertexInputAttribute>::operator()(const VertexInputAttribute &obj) const {
    ...
    bool equal_to<VertexInputAttribute>::
    operator()(const VertexInputAttribute &first, const VertexInputAttribute &second) const
      noexcept {
        return vertex_attribute_hasher(first) == vertex_attribute_hasher(second);
    }

    std::size_t hash<VertexInputBinding>::operator()(const VertexInputBinding &obj) const {
    ...
    bool equal_to<VertexInputBinding>::
    operator()(const VertexInputBinding &first, const VertexInputBinding &second) const noexcept {
