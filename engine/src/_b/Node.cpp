...
void Node::removeChild(const Node::Ptr& child) {
	...
	auto parent_it = std::find(std::begin(child->m_parents), std::end(child->m_parents), shared_from_this());
...
void Node::notify(NodeEvent event) noexcept {
	Subject::notify(std::move(event), std::make_pair<>(shared_from_this(), dirtyValue() | UPDATE_NODE_STATE));
}
