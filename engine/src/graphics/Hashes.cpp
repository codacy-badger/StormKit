...
namespace std {
	...
	bool equal_to<Vertex_P>::operator()(
	    const Vertex_P &first, const Vertex_P &second) const noexcept {
	...
	bool equal_to<Vertex_P_C>::operator()(
	    const Vertex_P_C &first, const Vertex_P_C &second) const noexcept {
	...
	bool equal_to<Vertex_P_C_N>::operator()(
	    const Vertex_P_C_N &first, const Vertex_P_C_N &second) const noexcept {
		return vertex_p_c_n_hasher(first) == vertex_p_c_n_hasher(second);
	}

	std::size_t hash<Vertex_P_C_UV>::operator()(
	    const Vertex_P_C_UV &obj) const {
		auto hash = vec3_hasher(obj.position);
		hash_combine(hash, vec4_hasher(obj.color));
	...
	bool equal_to<Vertex_P_C_UV>::operator()(const Vertex_P_C_UV &first,
	    const Vertex_P_C_UV &second) const noexcept {
	...
	bool equal_to<UniformBufferBinding>::operator()(
	    const UniformBufferBinding &first,
	    const UniformBufferBinding &second) const noexcept {
		return uniform_buffer_binding_hasher(first)
		       == uniform_buffer_binding_hasher(second);
	}

	std::size_t hash<TextureBinding>::operator()(
	    const TextureBinding &obj) const {
	...
	bool equal_to<TextureBinding>::operator()(const TextureBinding &first,
	    const TextureBinding &second) const noexcept {
	...
	bool equal_to<BindingState>::operator()(
	    const BindingState &first, const BindingState &second) const noexcept {
