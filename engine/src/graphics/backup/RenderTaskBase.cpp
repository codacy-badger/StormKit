...
RenderTaskBase::RenderTaskBase(std::string name, RenderGraph &parent)
	: m_name{std::move(name)}, m_parent{parent}, m_id{0u}, m_ref_count{0u}, m_framebuffer{nullptr} {
...
void RenderTaskBase::initRenderPass(ResourcePool &pool) {
	...
	m_framebuffer = Framebuffer::makeUnique(m_parent.get().device(), m_render_pass->implementation());
}

uvec2 RenderTaskBase::computeSize(const std::optional<std::reference_wrapper<const ResourceBase>> &depth, const std::vector<std::reference_wrapper<const ResourceBase>> &colors) {
	auto size = uvec2{};

	if(depth) {
		auto &depth_resource = static_cast<const TextureResource&>(depth.value().get());

		size = {depth_resource.description().size.x, depth_resource.description().size.y};
	} else {
		auto &color_resource = static_cast<const TextureResource&>(colors[0].get());

		size = {color_resource.description().size.x, color_resource.description().size.y};
	}
