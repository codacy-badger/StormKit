...
void RenderGraph::compile() {
	 ...
	 * reads Add them to the stack when their refCount== 0
	 */

	const auto get_resource_func = [this](auto id) -> ResourceBase & {
		return m_resources.getResource(id);
	};
	const auto get_render_task_func
	    = [this](auto id) -> RenderTaskBase & { return getRenderTask(id); };

	for (auto &render_task : m_render_tasks)
		render_task->m_ref_count = std::size(render_task->m_create_attachments)
		                           + std::size(render_task->m_write_attachments)
		                           + std::size(render_task->m_create_buffers)
		                           + std::size(render_task->m_write_buffers);

	const auto updateRefs = [](auto &resource) {
		resource->m_ref_count = std::size(resource->m_readers);
	};
	std::for_each(std::begin(m_resources.textures()),
	    std::end(m_resources.textures()), updateRefs);
	std::for_each(std::begin(m_resources.buffers()),
	    std::end(m_resources.buffers()), updateRefs);

	auto unreferenced_resources
	    = std::stack<std::reference_wrapper<ResourceBase>> {};

	const auto initUnreferencedResources
	    = [&unreferenced_resources](const auto &resource) {
		      if (resource->m_ref_count == 0 && resource->transient())
			      unreferenced_resources.emplace(*resource);
	      };

	std::for_each(std::cbegin(m_resources.textures()),
	    std::cend(m_resources.textures()), initUnreferencedResources);
	std::for_each(std::cbegin(m_resources.buffers()),
	    std::cend(m_resources.buffers()), initUnreferencedResources);
	...
	const auto update_create_resources
	    = [&realized_resources, &derealized_resources, &get_resource_func](
	...
	const auto update_rw_resources =
	    [&realized_resources, &derealized_resources](const auto &read_resources,
	        const auto &write_resources, auto &reads_writes) {
		    reads_writes.reserve(
		        std::size(read_resources) + std::size(write_resources));

		    std::copy(std::cbegin(read_resources), std::cend(read_resources),
		        std::back_inserter(reads_writes));
		    std::copy(std::cbegin(write_resources), std::cend(write_resources),
		        std::back_inserter(reads_writes));
	    };
	...
	for (auto &render_task_ptr : m_render_tasks) {
		...
		update_rw_resources(render_task.m_read_attachments,
		    render_task.m_write_attachments, read_write_attachments);
		update_rw_resources(render_task.m_read_buffers,
		    render_task.m_write_buffers, read_write_buffers);

		const auto cull = [this, &render_task, &derealized_resources,
		                      &get_resource_func](auto &reads_writes) {
			for (auto resource_id : reads_writes) {
				...
				if (!resource.m_writers.empty()) {
					...
					if (it != std::cend(m_render_tasks)) {
						valid      = true;
						last_index = std::max(
						    last_index, static_cast<std::size_t>(std::distance(
						                    std::cbegin(m_render_tasks), it)));
					}
				}

				if (valid
				    && m_render_tasks[last_index]->id() == render_task.id())
					derealized_resources.emplace_back(resource_id);
			}
		};
...
void RenderGraph::exportGraphviz(const _std::filesystem::path &filepath) const {
	const auto get_render_task_func =
	    [this](auto id) -> const RenderTaskBase & { return getRenderTask(id); };
	const auto get_resource_func = [this](auto id) -> const ResourceBase & {
		return m_resources.getResource(id);
	};

	const auto print_resource_func
	    = [&get_resource_func](auto &stream, auto &resources) {
	...
	const auto print_whole_resource_func = [](auto &       stream,
	                                           const auto &resources) {
	...
	const auto print_readers_func
	    = [&get_render_task_func](auto &stream, const auto &resources) {
		      for (const auto &resource : resources) {
			      stream << "\"" << resource->name() << "\" -> {";
