...
void SceneTree::onNotified(NodeEvent event, NodePayload &&payload) {
    switch(event) {
        case NodeEvent::ADDED: addNode(std::move(payload)); break;
        case NodeEvent::REMOVED: removeNode(std::move(payload)); break;
        case NodeEvent::UPDATED: updateNode(std::move(payload)); break;
    }
}

////////////////////////////////////////
////////////////////////////////////////
void SceneTree::exportSceneTree(
...
void SceneTree::traverse(MeshList &mesh_list) {
    ...
    while(!m_dirties.empty()) {
        ...
        if(
          std::find(std::begin(dirties), std::end(dirties), current_dirty_node) !=
          std::end(dirties))
            continue;
        ...
        for(auto parent_node = m_tree[ current_dirty_node ].parent();
            ...
            if(
              std::find(std::begin(dirties), std::end(dirties), parent_node) != std::end(dirties)) {
            ...
            if(m_tree[ parent_node ].dirtyBits()) {
                current_dirty_node = parent_node;
                m_dirties.erase(
                  std::find(std::begin(m_dirties), std::end(m_dirties), current_dirty_node));
            }
        }
    ...
    for(const auto index : dirties) traverseSubTree(index, mesh_list);
...
void SceneTree::traverseSubTree(tools::TreeNode::Index index, MeshList &mesh_list) {
    ...
    while(!stack.empty()) {
        ...
        switch(dirty_bits) {
            ...
            case GEOMETRY_BITS | CREATE_NODE_STATE: {
                ...
                const auto vertex_buffer_desc = HardwareBuffer::Description{
                  data.vertices.size, data.vertices.alignement, BufferUsageFlag::VERTEX};

                mesh.get().vertex_buffer =
                  m_device.get().createHardwareBufferPtr(std::move(vertex_buffer_desc));
                ...
                if(data.indices.size) {
                    const auto index_buffer_desc = HardwareBuffer::Description{
                      data.indices.size, data.indices.alignement, BufferUsageFlag::INDEX};

                    mesh.get().index_buffer =
                      m_device.get().createHardwareBufferPtr(std::move(index_buffer_desc));

                    std::visit(
                      core::overload{[&](const IndexArray &indices) {
        ...
        for(auto child = current_tree_node.firstChild(); child != tools::TreeNode::INVALID_INDEX;
            child      = m_tree[ child ].nextSibling()) {
            if(m_tree[ child ].dirtyBits() == 0)
                m_tree.markDirty(
                  child, m_graph_tree_link.at(child).get().dirtyValue() | UPDATE_NODE_STATE);
...
void SceneTree::addNode(NodePayload &&payload) {
    ...
    if(!parents.size()) {
        ...
        auto node_index =
          m_tree.insert(node, tools::TreeNode::INVALID_INDEX, tools::TreeNode::INVALID_INDEX);
        ...
        for(const auto &parent : parents) {
            ...
            for(const auto parent_tree_index : parent_tree_indices) {
                ...
                node.setName(payload.node.get().name());
                auto node_index =
                  m_tree.insert(node, parent_tree_index, tools::TreeNode::INVALID_INDEX);
                m_tree.markDirty(node_index, payload.node.get().dirtyValue() | CREATE_NODE_STATE);
                m_graph_tree_link.emplace(node_index, std::move(payload.node));
            }
        }
...
void SceneTree::updateNode(NodePayload &&payload) {
    ...
    std::for_each(
      ...
      [&payload, &tree_nodes](const auto &data) {
          if(&data.second.get() == &payload.node.get()) tree_nodes.push_back(data.first);
      });
