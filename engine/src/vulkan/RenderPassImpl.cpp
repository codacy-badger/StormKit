...
void RenderPassImpl::createRenderPass() {
	...
	for(const auto &attachment : std::as_const(m_attachments)) {
		...
		if(isDepthFormat(asVK(attachment)))
			depth_stencil_attachment_index = std::size(attachment_descriptions) - 1;
	}

	auto subpass_descriptions = std::vector<vk::SubpassDescription>{};
	auto input_attachments = std::vector<std::vector<vk::AttachmentReference>>{};
	auto output_attachments = std::vector<std::vector<vk::AttachmentReference>>{};
	...
	for(const auto &subpass : m_subpasses) {
		...
		for(auto input_attachment : subpass.input_attachments)
			input_attachments_subpass.emplace_back(attachment_references[input_attachment]);

		for(auto output_attachment : subpass.output_attachments) {
			if(output_attachment == depth_stencil_attachment_index)
				subpass_description.setPDepthStencilAttachment(&attachment_references[output_attachment]);
			else
				output_attachments_subpass.emplace_back(attachment_references[output_attachment]);
		}
	...
	storm::DLOG("Renderer (vulkan)"_module, "Render pass at %{1} allocated", &m_render_pass.get());
}
