...
#define AS(type, map)                                                                            \
    const auto iterator = std::find_if(                                                          \
      std::begin(map), std::end(map), [type](const auto &pair) { return pair.second == type; }); \
    return iterator->first;

#define FROM(type, map)                                                                         \
    const auto iterator = std::find_if(                                                         \
      std::begin(map), std::end(map), [type](const auto &pair) { return pair.first == type; }); \
    return iterator->second;

    static inline constexpr const auto SHADER_TYPE_MAP = MappingArray<
      vk::ShaderStageFlagBits,
      storm::engine::Shader::Stage,
    ...
    static inline constexpr const auto TOPOLOGY_MAP = MappingArray<
    ...
    static inline constexpr const auto POLYGON_MODE_MAP = MappingArray<
      vk::PolygonMode,
      storm::engine::PolygonMode,
      static_cast<std::size_t>(storm::engine::PolygonMode::SIZE)>{
      std::pair<vk::PolygonMode, storm::engine::PolygonMode>{vk::PolygonMode::eFill,
                                                             storm::engine::PolygonMode::FILL},
      {vk::PolygonMode::eLine, storm::engine::PolygonMode::LINE},
      {vk::PolygonMode::ePoint, storm::engine::PolygonMode::POINT},
    };

    vk::PolygonMode asVK(storm::engine::PolygonMode mode) noexcept {AS(mode, POLYGON_MODE_MAP)}

    storm::engine::PolygonMode fromVK(vk::PolygonMode mode) noexcept {
        FROM(mode, POLYGON_MODE_MAP)
    }

    static inline const auto CULL_MODE_MAP = MappingArray<
      vk::CullModeFlags,
      storm::engine::CullMode,
      static_cast<std::size_t>(storm::engine::CullMode::SIZE)>{
      std::pair<vk::CullModeFlags, storm::engine::CullMode>{vk::CullModeFlagBits::eBack,
                                                            storm::engine::CullMode::BACK},
    ...
    vk::CullModeFlags asVK(storm::engine::CullMode mode) noexcept {AS(mode, CULL_MODE_MAP)}

    storm::engine::CullMode fromVK(vk::CullModeFlags mode) noexcept {
        FROM(mode, CULL_MODE_MAP)
    }

    static inline constexpr const auto FRONT_FACE_MAP = MappingArray<
      vk::FrontFace,
      storm::engine::FrontFace,
      static_cast<std::size_t>(storm::engine::FrontFace::SIZE)>{
      std::pair<vk::FrontFace, storm::engine::FrontFace>{vk::FrontFace::eClockwise,
                                                         storm::engine::FrontFace::CLOCKWISE},
      {vk::FrontFace::eCounterClockwise, storm::engine::FrontFace::COUNTER_CLOCKWISE},
    };

    vk::FrontFace asVK(storm::engine::FrontFace face) noexcept {AS(face, FRONT_FACE_MAP)}

    storm::engine::FrontFace fromVK(vk::FrontFace face) noexcept {FROM(face, FRONT_FACE_MAP)}

    vk::ColorComponentFlags asVK(storm::engine::ColorComponentFlag color) noexcept {
        auto _color = vk::ColorComponentFlags{};

        if((color & storm::engine::ColorComponentFlag::R) == storm::engine::ColorComponentFlag::R)
            _color |= vk::ColorComponentFlagBits::eR;
        if((color & storm::engine::ColorComponentFlag::G) == storm::engine::ColorComponentFlag::G)
            _color |= vk::ColorComponentFlagBits::eG;
        if((color & storm::engine::ColorComponentFlag::B) == storm::engine::ColorComponentFlag::B)
            _color |= vk::ColorComponentFlagBits::eB;
        if((color & storm::engine::ColorComponentFlag::A) == storm::engine::ColorComponentFlag::A)
            _color |= vk::ColorComponentFlagBits::eA;

        return _color;
    }

    storm::engine::ColorComponentFlag fromVK(vk::ColorComponentFlags color) noexcept {
        auto _color = storm::engine::ColorComponentFlag{};

        if(color & vk::ColorComponentFlagBits::eR) _color |= storm::engine::ColorComponentFlag::R;
        if(color & vk::ColorComponentFlagBits::eG) _color |= storm::engine::ColorComponentFlag::G;
        if(color & vk::ColorComponentFlagBits::eB) _color |= storm::engine::ColorComponentFlag::B;
        if(color & vk::ColorComponentFlagBits::eA) _color |= storm::engine::ColorComponentFlag::A;

        return _color;
    }

    static inline constexpr const auto PIPELINE_STATE_MAP = MappingArray<
    ...
    static inline constexpr const auto VERTEX_INPUT_RATE_MAP = MappingArray<
      vk::VertexInputRate,
      storm::engine::VertexInputRate,
    ...
    static inline constexpr const auto FORMAT_MAP = MappingArray<
      vk::Format,
      storm::engine::Format,
      static_cast<std::size_t>(storm::engine::Format::SIZE)>{
      std::pair<vk::Format, storm::engine::Format>{vk::Format::eR32Sfloat,
                                                   storm::engine::Format::FLOAT},
    ...
    static inline constexpr const auto COMPARE_OP_MAP = MappingArray<
      vk::CompareOp,
      storm::engine::CompareOperation,
      static_cast<std::size_t>(storm::engine::CompareOperation::SIZE)>{
      std::pair<vk::CompareOp, storm::engine::CompareOperation>{
        vk::CompareOp::eNever,
        storm::engine::CompareOperation::NEVER},
    ...
    vk::CompareOp asVK(storm::engine::CompareOperation op) noexcept {AS(op, COMPARE_OP_MAP)}

    storm::engine::CompareOperation fromVK(vk::CompareOp op) noexcept {FROM(op, COMPARE_OP_MAP)}
    ...
    vk::ImageUsageFlags asVK(storm::engine::ImageUsageFlag usage) noexcept {
        ...
        if(
          (usage & ImageUsageFlag::DEPTH_STENCIL_ATTACHMENT) ==
          ImageUsageFlag::DEPTH_STENCIL_ATTACHMENT)
    ...
    static inline constexpr const auto IMAGE_LAYOUT_MAP = MappingArray<
      vk::ImageLayout,
      storm::engine::ImageLayout,
      static_cast<std::size_t>(storm::engine::ImageLayout::SIZE)>{
      std::pair<vk::ImageLayout, storm::engine::ImageLayout>{
        vk::ImageLayout::eColorAttachmentOptimal,
        storm::engine::ImageLayout::COLOR_ATTACHMENT_OPTIMAL},
      {vk::ImageLayout::eDepthStencilAttachmentOptimal,
       storm::engine::ImageLayout::DEPTH_STENCIL_ATTACHMENT_OPTIMAL},
      {vk::ImageLayout::eDepthStencilReadOnlyOptimal,
       storm::engine::ImageLayout::DEPTH_STENCIL_READ_ONLY_OPTIMAL},
      ...
      {vk::ImageLayout::eShaderReadOnlyOptimal,
       storm::engine::ImageLayout::SHADER_READ_ONLY_OPTIMAL},
    };

    vk::ImageLayout asVK(storm::engine::ImageLayout layout) noexcept {AS(layout, IMAGE_LAYOUT_MAP)}

    storm::engine::ImageLayout
      fromVK(vk::ImageLayout layout) noexcept {FROM(layout, IMAGE_LAYOUT_MAP)}
