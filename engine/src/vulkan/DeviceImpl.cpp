...
/////////////////////////////////////
/////////////////////////////////////
BackedSwapchainImage
DeviceImpl::createBackedSwapchainImage(vk::Format format, vk::Image image) const {
    ...
    const auto subressource_range = vk::ImageSubresourceRange{}
    ...
    return BackedSwapchainImage{image, std::move(image_view)};
}

/////////////////////////////////////
/////////////////////////////////////
BackedVkImage DeviceImpl::createBackedVkImage(
  ...
  vk::MemoryPropertyFlags memory_properties) const {
    ...
    const auto image_memory_allocate_info =
      vk::MemoryAllocateInfo{}
        ...
        .setMemoryTypeIndex(
          findMemoryRequirement(memory_requirements.memoryTypeBits, memory_properties));

    auto device_memory = m_logical_device->allocateMemoryUnique(image_memory_allocate_info);

    storm::DLOG(
      "Renderer (vulkan)"_module, "Device Memory at %{1} allocated", &device_memory.get());
    ...
    auto submit_info = vk::SubmitInfo{}.setCommandBufferCount(1).setPCommandBuffers(
    ...
    return {std::move(image),
            ...
            {infos.extent.width, infos.extent.height},
            layout};
}
...
/////////////////////////////////////
/////////////////////////////////////
BackedVkBuffer DeviceImpl::createBackedVkBuffer(
  vk::BufferUsageFlags usage,
  vk::MemoryPropertyFlags memory_properties,
  vk::DeviceSize size) const {
    const auto buffer_info = vk::BufferCreateInfo{}.setSize(size).setUsage(usage).setSharingMode(
...
UniqueHandle<vk::CommandBuffer>
DeviceImpl::allocateCommandBuffer(DeviceImpl::QueueType type, vk::CommandBufferLevel level) const {
    ...
    auto command_buffers =
      m_logical_device->allocateCommandBuffersUnique(allocate_command_buffer_info);

    storm::DLOG(
      "Renderer (vulkan)"_module, "Command Buffer allocated at %{1}", &command_buffers[ 0 ].get());
...
/////////////////////////////////////
std::int32_t
DeviceImpl::findMemoryRequirement(std::uint32_t filter, vk::MemoryPropertyFlags properties) const {
...
void DeviceImpl::createLogicalDevice() {
    ...
    if(m_family_queue_indices.transfert_queue_index != 255) {
        device_create_info.setEnabledExtensionCount(1).setPpEnabledExtensionNames(
          std::data(ContextImpl::DEVICE_EXTENSIONS));
    }

    const auto &vk_physical_device = m_physical_device.implementation().vkPhysicalDevice();
    m_logical_device               = vk_physical_device.createDeviceUnique(device_create_info);
    if(!m_logical_device) throw std::runtime_error("Failed to create logical device");

    if(m_family_queue_indices.graphics_queue_index != 255)
        m_graphics_queue =
          m_logical_device->getQueue(m_family_queue_indices.graphics_queue_index, 0);
    ...
    if(m_family_queue_indices.transfert_queue_index != 255)
        m_transfert_queue =
          m_logical_device->getQueue(m_family_queue_indices.transfert_queue_index, 0);

    storm::DLOG(
      "Renderer (vulkan)"_module, "Logical Device allocated at %{1}", &m_logical_device.get());
}
    ...
    for(const auto &queue_family : std::as_const(queue_families)) {
        if(queue_family.queueCount > 0) {
            ...
            const auto visual_id =
              xcb_setup_roots_iterator(xcb_get_setup(handles->connection)).data->root_visual;
            const auto present_support = vkGetPhysicalDeviceXcbPresentationSupportKHR(
              vk_physical_device, current_index, handles->connection, visual_id);
#elif defined(VK_USE_PLATFORM_MACOS_MVK)
            const auto present_support = true;
#endif

            if(present_support) m_family_queue_indices.transfert_queue_index = current_index;
...
void DeviceImpl::createCommandPools() {
    ...
    m_command_pools[ 0 ] =
      m_logical_device->createCommandPoolUnique(graphics_command_pool_create_info);
    storm::DLOG(
      "Renderer (vulkan)"_module, "Command Pool allocated at %{1}", &m_command_pools[ 0 ].get());
    ...
    m_command_pools[ 1 ] =
      m_logical_device->createCommandPoolUnique(compute_command_pool_create_info);
    storm::DLOG(
      "Renderer (vulkan)"_module, "Command Pool allocated at %{1}", &m_command_pools[ 1 ].get());
    ...
    m_command_pools[ 2 ] =
      m_logical_device->createCommandPoolUnique(transfert_command_pool_create_info);
    storm::DLOG(
      "Renderer (vulkan)"_module, "Command Pool allocated at %{1}", &m_command_pools[ 2 ].get());
}
...
vk::Format DeviceImpl::findSupportedFormat(
  ...
  vk::FormatFeatureFlags features) const noexcept {
    for(auto format : candidates) {
        auto properties =
          m_physical_device.implementation().vkPhysicalDevice().getFormatProperties(format);
