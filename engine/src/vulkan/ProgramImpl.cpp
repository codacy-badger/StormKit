...
/////////////////////////////////////
/////////////////////////////////////
ProgramImpl::ProgramImpl(const Device &device)
	: m_device{device} {
...
void ProgramImpl::link() {
	...
	auto vertex_shader = std::find_if(std::begin(m_shaders), std::end(m_shaders), [](const auto &shader) { return shader->stage() == Shader::Stage::VERTEX; });
	
	auto descriptor_layout_bindings = std::vector<vk::DescriptorSetLayoutBinding>{};
	if(vertex_shader != std::end(m_shaders)) {
		auto spirv_binary = (*vertex_shader)->source();
		auto spirv_binary_int = std::vector<std::uint32_t>{};
		spirv_binary_int.resize(std::size(spirv_binary) / sizeof(std::uint32_t), 0);

		std::memcpy(std::data(spirv_binary_int), std::data(spirv_binary), std::size(spirv_binary));
		...
		for(const auto &uniform : shader_resources.uniform_buffers) {
			const auto set = compiler.get_decoration(uniform.id, spv::DecorationDescriptorSet);
			const auto binding = compiler.get_decoration(uniform.id, spv::DecorationBinding);

			const auto descriptor_layout_binding = vk::DescriptorSetLayoutBinding{}
					.setBinding(binding)
	...
	auto fragment_shader = std::find_if(std::begin(m_shaders), std::end(m_shaders), [](const auto &shader) { return shader->stage() == Shader::Stage::FRAGMENT; });
	...
	m_descriptor_set_layouts.emplace_back(vk_device.createDescriptorSetLayoutUnique(descriptor_set_layout_create_info));
	...
	m_pipeline_layout = vk_device.createPipelineLayoutUnique(pipeline_layout_create_info);
}
