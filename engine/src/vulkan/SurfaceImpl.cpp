...
/////////////////////////////////////
/////////////////////////////////////
void SurfaceImpl::presentFrame(const Framebuffer &framebuffer, const Semaphore &render_finished_semaphore, const Fence &signal_fence) {
	m_swapchain->presentFrame(framebuffer, render_finished_semaphore, signal_fence);
}
...
void SurfaceImpl::createSurface() {
	...
	const auto &context            = device.context().implementation();
	const auto &vk_instance        = context.instance();
	const auto &vk_physical_device = device.physicalDevice().implementation().vkPhysicalDevice();
	...
	m_surface = vk_instance.createXcbSurfaceKHR(create_info, nullptr, context.dispatcher());
#elif defined(VK_USE_PLATFORM_MACOS_MVK)
	auto create_info = vk::MacOSSurfaceCreateInfoMVK{}
			.setPView(m_window.nativeHandle());

	m_surface = vk_instance.createMacOSSurfaceMVK(create_info, nullptr, context.dispatcher());
#endif

	if(!m_surface)
		throw std::runtime_error("Failed to create Vulkan surface");

	storm::DLOG("Renderer (vulkan)"_module, "Render surface allocated at %{1}", &m_surface);
	
	m_surface_capabilities = vk_physical_device.getSurfaceCapabilitiesKHR(m_surface);
