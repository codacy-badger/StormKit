...
ShaderImpl::ShaderImpl(const Device &device, Shader::DefaultShader shader)
	...
	switch(shader) {
		case Shader::DefaultShader::DEFAULT_VERT_2D:
			...
			loadFromMemory(reinterpret_cast<const std::byte*>(default_vert_2D), sizeof(default_vert_2D));
			break;
		case Shader::DefaultShader::DEFAULT_FRAG_2D:
			m_stage = Shader::Stage::FRAGMENT;
			loadFromMemory(reinterpret_cast<const std::byte*>(default_frag_2D), sizeof(default_frag_2D));
			break;
	}
}
...
/////////////////////////////////////
/////////////////////////////////////
ShaderImpl::ShaderImpl(const Device &device, Shader::Stage stage, const _std::filesystem::path &path)
...
/////////////////////////////////////
/////////////////////////////////////
ShaderImpl::ShaderImpl(const Device &device, Shader::Stage stage, const std::byte* data, std::size_t size)
	: m_stage{stage}, m_device{device} {
...
void ShaderImpl::loadFromFile(const _std::filesystem::path &path) {
	...
	loadFromMemory(reinterpret_cast<std::byte*>(std::data(data)), std::size(data));
}
...
void ShaderImpl::loadFromMemory(const std::byte* data, std::size_t size) {
	...
	const auto create_info = vk::ShaderModuleCreateInfo{}
				.setCodeSize(size)
				.setPCode(reinterpret_cast<const std::uint32_t*>(data));

	m_shader_module = vk_device.createShaderModuleUnique(create_info);

	storm::DLOG("Renderer (vulkan)"_module, "Shader module allocated at %{1}", &m_shader_module.get());
