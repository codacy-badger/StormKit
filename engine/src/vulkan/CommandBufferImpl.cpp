...
static const auto access_map = std::unordered_map<vk::ImageLayout, vk::AccessFlags>{
...
CommandBufferImpl::CommandBufferImpl(const Device &device, bool primary)
	: m_current_render_pass{nullptr}, 
	  ...
	  m_is_primary_command_buffer{primary} {
	createCommandBuffer();
...
void CommandBufferImpl::begin(bool one_time_submit) const {
	if(one_time_submit) m_command_buffer->begin({vk::CommandBufferUsageFlagBits::eOneTimeSubmit});
	else m_command_buffer->begin({vk::CommandBufferUsageFlagBits::eSimultaneousUse});
}

/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::end() const {
...
void CommandBufferImpl::beginRenderPass(RenderPass &render_pass, const Framebuffer &buffer, core::RGBColorF clear_color) {
	...
	if(m_current_render_pass->implementation().hasDepthAttachment()) {
		const auto depth_color_value = vk::ClearDepthStencilValue{}
									   .setDepth(1.f)
									   .setStencil(0.f);
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::draw(std::size_t vertex_count, std::size_t instance_count, 
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::drawIndexed(std::size_t index_count, std::size_t instance_count, 
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::submit(const std::vector<const Semaphore*> &wait_semaphores, const std::vector<const Semaphore*> &signal_semaphores, std::vector<PipelineStage> pipeline_states, const Fence *fence) {
	const auto &vk_device = m_device.implementation().vkDevice();

	auto render_finished_fence = (fence) ? fence->implementation().vkFence() : vk::Fence{nullptr};
	...
	for(const auto &semaphore : wait_semaphores)
		if(semaphore)
			wait_vk_semaphores.emplace_back(semaphore->implementation().vkSemaphore());
	...
	for(const auto &semaphore : signal_semaphores)
		if(semaphore)
			signal_vk_semaphores.emplace_back(semaphore->implementation().vkSemaphore());
	...
	m_device.implementation().vkGraphicsQueue().submit(submit_info, render_finished_fence);
}
...
void CommandBufferImpl::setProgram(const Program &program) {
	...
	m_current_pipeline_layout = &(m_current_program->implementation().vkPipelineLayout());
}

/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::transitionImageLayout(vk::Image image, vk::ImageLayout old_layout, vk::ImageLayout new_layout, vk::PipelineStageFlagBits source, vk::PipelineStageFlagBits destination, vk::ImageSubresourceRange range) {
	 auto src_access = access_map.find(old_layout);
	 auto dst_access = access_map.find(new_layout);

	 if (src_access == std::end(access_map) || dst_access == std::end(access_map)) {
	...
	m_command_buffer->pipelineBarrier(source, destination, vk::DependencyFlagBits(), nullptr, nullptr, barrier);
}

/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::blitImage(vk::Image source, vk::Image destination, const uvec2 &src_size,
								  const uvec2 &destination_size, vk::Filter filter) const {
	blitImage(source, 0, destination, 0, src_size, destination_size, 1, filter);
}

/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::blitImage(vk::Image source, uint32_t source_mipmap_level, vk::Image destination,
								  uint32_t destination_mipmap_level, const uvec2 &src_size, const uvec2 &destination_size,
								  uint32_t layer_count, vk::Filter filter) const {

	const auto source_sub_ressource = vk::ImageSubresourceLayers{}
			.setAspectMask(vk::ImageAspectFlagBits::eColor)
			.setLayerCount(layer_count)
			.setMipLevel(source_mipmap_level);

	const auto destination_sub_ressource = vk::ImageSubresourceLayers{}
			.setAspectMask(vk::ImageAspectFlagBits::eColor)
			.setLayerCount(layer_count)
			.setMipLevel(destination_mipmap_level);

	const auto info = vk::ImageBlit{}
			.setSrcSubresource(source_sub_ressource)
			.setDstSubresource(destination_sub_ressource)
			.setSrcOffsets({ vk::Offset3D{0, 0, 0}, vk::Offset3D{static_cast<std::int32_t>(src_size.x), static_cast<std::int32_t>(src_size.y), 1} })
			.setDstOffsets({ vk::Offset3D{0, 0, 0}, vk::Offset3D{static_cast<std::int32_t>(destination_size.x), static_cast<std::int32_t>(destination_size.y), 1} });

  m_command_buffer->blitImage(source, vk::ImageLayout::eTransferSrcOptimal, destination,
							  vk::ImageLayout::eTransferDstOptimal, info, filter);
}

/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::copyBuffer(const vk::Buffer &src, const vk::Buffer &dest, vk::DeviceSize size) const {
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::copyBufferToImage(const vk::Buffer &src, const vk::Image &dest, std::uint32_t width, std::uint32_t height) const {
	...
	m_command_buffer->copyBufferToImage(src, dest, vk::ImageLayout::eTransferDstOptimal, { copy_info });
}

/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::bindVertexBuffer(std::uint32_t index, const VertexBuffer &buffer) {
	m_command_buffer->bindVertexBuffers(index, { buffer.implementation().backedVkBuffer().buffer.get() }, { 0 });
}

void CommandBufferImpl::bindIndexBuffer(const IndexBuffer &buffer, bool large_indices) {
	const auto &vk_buffer = buffer.implementation().backedVkBuffer().buffer.get();
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::executeCommandBuffers(const std::vector<std::reference_wrapper<CommandBuffer>> &command_buffers) {
	auto vk_command_buffers = std::vector<vk::CommandBuffer>{};
	vk_command_buffers.reserve(std::size(command_buffers));
	
	for(const auto &command_buffer : command_buffers) {
		vk_command_buffers.emplace_back(command_buffer.get().implementation().vkCommandBuffer());
	}
	
	m_command_buffer->executeCommands(std::size(vk_command_buffers), std::data(vk_command_buffers));
}
...
void CommandBufferImpl::createCommandBuffer() {
	...
	m_command_buffer = device.allocateCommandBuffer(DeviceImpl::QueueType::GRAPHICS, 
		(m_is_primary_command_buffer) ? vk::CommandBufferLevel::ePrimary : vk::CommandBufferLevel::eSecondary);
}
...
const vk::Pipeline &CommandBufferImpl::getOrCreatePipeline() {
	...
	auto shaders_create_info = std::vector<vk::PipelineShaderStageCreateInfo>{};
	...
	vertex_input_bindings.reserve(std::size(m_pipeline_state.vertex_input_state.bindings));
	vertex_input_attributes.reserve(std::size(m_pipeline_state.vertex_input_state.attributes));

	for(const auto &binding : m_pipeline_state.vertex_input_state.bindings)
		vertex_input_bindings.emplace_back(binding.binding, binding.stride, asVK(binding.rate));

	for(const auto &attribute : m_pipeline_state.vertex_input_state.attributes)
		vertex_input_attributes.emplace_back(attribute.location, attribute.binding, asVK(attribute.type), attribute.offset);

	const auto vertex_input_info = vk::PipelineVertexInputStateCreateInfo{}
			.setVertexBindingDescriptionCount(std::size(vertex_input_bindings))
			.setVertexAttributeDescriptionCount(std::size(vertex_input_attributes))
			.setPVertexBindingDescriptions(std::data(vertex_input_bindings))
			.setPVertexAttributeDescriptions(std::data(vertex_input_attributes));

	const auto vertex_input_assembly_info = vk::PipelineInputAssemblyStateCreateInfo{}
	...
	const auto scissor = vk::Rect2D{}
			.setOffset({m_pipeline_state.scissor.x, m_pipeline_state.scissor.y})
			.setExtent({static_cast<std::uint32_t>(m_pipeline_state.scissor.z), static_cast<std::uint32_t>(m_pipeline_state.scissor.w)});
	...
	const auto pipeline_rasterizer_info = vk::PipelineRasterizationStateCreateInfo{}
	...
	const auto pipeline_multisampling_info = vk::PipelineMultisampleStateCreateInfo{}
			.setSampleShadingEnable(false)
			.setRasterizationSamples(vk::SampleCountFlagBits::e1);

	const auto pipeline_color_blending_info = vk::PipelineColorBlendAttachmentState{}
			.setColorWriteMask(asVK(m_pipeline_state.blend_write_color_mask))
			.setBlendEnable(m_pipeline_state.is_blending_enabled);

	const auto pipeline_color_blending_state = vk::PipelineColorBlendStateCreateInfo{}
			.setLogicOpEnable(false)
			.setAttachmentCount(1)
			.setPAttachments(&pipeline_color_blending_info);
	
	const auto pipeline_depth_stencil_state = vk::PipelineDepthStencilStateCreateInfo{}
	...
	auto pipeline_create_info = vk::GraphicsPipelineCreateInfo{}
			...
			.setRenderPass(m_current_render_pass->implementation().vkRenderPass())
			.setSubpass(0);

	const auto &vk_device = m_device.implementation().vkDevice();
	auto pipeline = vk_device.createGraphicsPipelineUnique(nullptr, pipeline_create_info);
	if(!pipeline)
		throw std::runtime_error("Failed to create & pipeline");

	storm::DLOG("Renderer (vulkan)"_module, "Pipeline allocated at %{1}", &pipeline.get());
...
/////////////////////////////////////
/////////////////////////////////////
bool PipelineCacheKey::operator==(const PipelineCacheKey &other) const noexcept {
	return std::hash<PipelineCacheKey>()(*this) == std::hash<PipelineCacheKey>()(other);
}
