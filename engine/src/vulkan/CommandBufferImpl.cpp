...
CommandBufferImpl::CommandBufferImpl(const Device &device, bool primary)
      : m_current_render_pass{nullptr}, m_current_program{nullptr}, m_device{device},
        m_is_pipeline_state_dirty{true}, m_is_binding_state_dirty{true},
...
void CommandBufferImpl::begin(bool one_time_submit) const {
    if(one_time_submit)
        ...
        m_command_buffer->begin({vk::CommandBufferUsageFlagBits::eSimultaneousUse});
}
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::beginRenderPass(
  ...
  core::RGBColorF clear_color) {
    ...
    if(buffer.hasDepthAttachment()) {
        const auto depth_color_value = vk::ClearDepthStencilValue{}.setDepth(1.f).setStencil(0.f);
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::drawIndexed(
...
void CommandBufferImpl::submit(
  ...
  const Fence *fence) {
    ...
    for(const auto &semaphore : wait_semaphores)
        if(semaphore) wait_vk_semaphores.emplace_back(semaphore->implementation().vkSemaphore());
    ...
    for(const auto &semaphore : signal_semaphores)
        if(semaphore) signal_vk_semaphores.emplace_back(semaphore->implementation().vkSemaphore());
    ...
    if(queue == QueueType::GRAPHICS)
        ...
        m_device.implementation().vkTransfertQueue().submit(submit_info, render_finished_fence);
    else if(queue == QueueType::COMPUTE)
        m_device.implementation().vkComputeQueue().submit(submit_info, render_finished_fence);
}
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::transitionImageLayout(
  vk::Image image,
  ...
  vk::ImageSubresourceRange range) {
    ...
    m_command_buffer->pipelineBarrier(
      source, destination, vk::DependencyFlagBits(), nullptr, nullptr, barrier);
}

/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::blitImage(
  vk::Image source,
  vk::Image destination,
  const uvec2 &src_size,
  const uvec2 &destination_size,
...
void CommandBufferImpl::blitImage(
  vk::Image source,
  uint32_t source_mipmap_level,
  vk::Image destination,
  uint32_t destination_mipmap_level,
  const uvec2 &src_size,
  const uvec2 &destination_size,
...
void CommandBufferImpl::copyBuffer(
  const vk::Buffer &src,
  const vk::Buffer &dest,
  vk::DeviceSize size) const {
    const auto copy_info = vk::BufferCopy{}.setSrcOffset(0).setDstOffset(0).setSize(size);
...
/////////////////////////////////////
/////////////////////////////////////
void CommandBufferImpl::bindVertexBuffer(std::uint32_t index, const HardwareBuffer &buffer) {
    m_command_buffer->bindVertexBuffers(
      index, {buffer.implementation().backedVkBuffer().buffer.get()}, {0});
}

void CommandBufferImpl::bindIndexBuffer(const HardwareBuffer &buffer, bool large_indices) {
...
void CommandBufferImpl::createCommandBuffer() {
    ...
    m_command_buffer = device.allocateCommandBuffer(
      DeviceImpl::QueueType::GRAPHICS,
...
void CommandBufferImpl::bindDescriptors() {
    ...
    m_command_buffer->bindDescriptorSets(
      vk::PipelineBindPoint::eGraphics, *m_current_pipeline_layout, 0, descriptor_sets, {});
}
...
const vk::Pipeline &CommandBufferImpl::getOrCreatePipeline() {
    ...
    if(pipeline_cache.has(key)) return pipeline_cache.acquire(key);
    ...
    for(const auto &attribute : m_pipeline_state.vertex_input_state.attributes)
        vertex_input_attributes.emplace_back(
          attribute.location, attribute.binding, asVK(attribute.type), attribute.offset);
    ...
    const auto scissor = vk::Rect2D{}
                           ...
                           .setExtent({static_cast<std::uint32_t>(m_pipeline_state.scissor.z),
    ...
    if(!pipeline) throw std::runtime_error("Failed to create & pipeline");
...
std::vector<vk::DescriptorSet> CommandBufferImpl::getOrCreateDescriptorSets() {
    ...
    if(m_is_binding_state_dirty) {
        ...
        m_descriptor_sets =
          device.vkDevice().allocateDescriptorSetsUnique(descriptor_set_alloc_info);
                  ...
                  core::overload{
                    [&](const UniformBufferBinding &buffer_binding) {
                        const auto &backed_buffer =
                          buffer_binding.buffer->implementation().backedVkBuffer();
                    ...
                    [&](const TextureBinding &image_binding) {
                        const auto &backed_texture =
                          image_binding.texture->implementation().backedVkTexture();
