...
, m_vulkan_shared_library {
    VULKAN_SHARED_LIBRARY_FILENAME
}
#endif
{ createInstance(); }
...
void ContextImpl::createInstance() {
    const auto vk_app_version = vkMakeVersion(
      m_context_settings.app_version.major,
      m_context_settings.app_version.minor,
      m_context_settings.app_version.patch);
    const auto vk_storm_version =
    ...
    if(!m_instance) throw std::runtime_error("Failed to create Vulkan instance!");

#ifndef STORM_OS_MACOS
    auto vk_get_instance_proc_addr_func =
      m_vulkan_shared_library.getCFunc<PFN_vkGetInstanceProcAddr>("vkGetInstanceProcAddr");
    ...
    if(enable_validation) {
        ...
        m_callback =
          m_instance->createDebugReportCallbackEXT(debug_create_info, nullptr, m_dispatcher);
        if(!m_callback) throw std::runtime_error("Failed to create Vulkan debug report output");
    }
...
std::uint32_t ContextImpl::scorePhysicalDevice(const vk::PhysicalDevice &device) const {
    ...
    if(properties.deviceType == vk::PhysicalDeviceType::eDiscreteGpu) score += 1000u;
