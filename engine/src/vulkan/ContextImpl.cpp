...
/////////////////////////////////////
/////////////////////////////////////
VKAPI_ATTR VkBool32 debugCallback(VkDebugReportFlagsEXT flags,
    VkDebugReportObjectTypeEXT objType, uint64_t obj, size_t location,
    int32_t code, const char *layerPrefix, const char *msg,
    [[maybe_unused]] void *userData) {
	static const constexpr auto MESSAGE
	    = "Layer: %{1} | Code: %{2}\n Message: %{3}";
	switch (flags) {
	case VK_DEBUG_REPORT_DEBUG_BIT_EXT:
		storm::DLOG(
		    "Renderer (vulkan-layer)"_module, MESSAGE, layerPrefix, code, msg);
		break;
...
/////////////////////////////////////
/////////////////////////////////////
ContextImpl::ContextImpl(ContextSettings settings) : m_context_settings {
	std::move(settings)
}
...
void ContextImpl::createInstance() {
	const auto vk_app_version
	    = vkMakeVersion(m_context_settings.app_version.major,
	        m_context_settings.app_version.minor,
	        m_context_settings.app_version.patch);
	const auto vk_storm_version = vkMakeVersion(
	...
	const auto enable_validation
	    = checkValidationLayerSupport(m_context_settings.enable_validation);
	if (enable_validation) {
		create_info
		    .setEnabledLayerCount(
		        static_cast<std::uint32_t>(std::size(VALIDATION_LAYERS)))
	...
	create_info
	    .setEnabledExtensionCount(
	        static_cast<std::uint32_t>(std::size(instance_extensions)))
...
#ifndef STORM_OS_MACOS
	auto vk_get_instance_proc_addr_func
	    = m_vulkan_shared_library.getCFunc<PFN_vkGetInstanceProcAddr>(
	...
	if (enable_validation) {
		...
		m_dispatcher.vkCreateDebugReportCallbackEXT
		    = reinterpret_cast<PFN_vkCreateDebugReportCallbackEXT>(
		        m_instance->getProcAddr("vkCreateDebugReportCallbackEXT"));
		m_dispatcher.vkDestroyDebugReportCallbackEXT
		    = reinterpret_cast<PFN_vkDestroyDebugReportCallbackEXT>(
		        m_instance->getProcAddr("vkDestroyDebugReportCallbackEXT"));

		m_callback = m_instance->createDebugReportCallbackEXT(
		    debug_create_info, nullptr, m_dispatcher);
		if (!m_callback)
			throw std::runtime_error(
			    "Failed to create Vulkan debug report output");
	}

	storm::DLOG("Renderer (vulkan)"_module, "Vulkan Instance allocated at %{1}",
	    &m_instance.get());
}
...
/////////////////////////////////////
/////////////////////////////////////
std::uint32_t ContextImpl::scorePhysicalDevice(
	...
	for (const auto &needed_extension : std::as_const(DEVICE_EXTENSIONS)) {
		...
		for (const auto &device_extension : std::as_const(device_extensions)) {
			has_extension
			    = std::strcmp(device_extension.extensionName, needed_extension)
			      == 0;
