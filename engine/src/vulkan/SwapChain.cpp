...
FrameToken SwapChain::nextFrame() {
    ...
    const auto image_index = vk_device
                               .acquireNextImageKHR(
                                 *m_swapchain,
    ...
    return FrameToken{
      m_current_frame, image_index, std::size(m_images), image_available, render_finished, fence};
}
...
void SwapChain::present(Framebuffer &framebuffer, const FrameToken &token) {
    ...
    const auto &framebuffer_image = framebuffer.implementation()
                                      .outputTextures()
                                      .begin()
    ...
    present_command_buffer.submit(
      {&token.render_finished, &token.image_available},
    ...
    device_impl.vkGraphicsQueue().presentKHR(
      vk::PresentInfoKHR{}
        ...
        .setPWaitSemaphores(
          &m_present_semaphores[ m_current_in_flight ].implementation().vkSemaphore()));
...
void SwapChain::createSwapChain() {
    ...
    m_device.implementation()
      ...
      .implementation()
      .vkPhysicalDevice()
      .getSurfaceSupportKHR(family_queue_indices.transfert_queue_index, vk_surface);
    ...
    if(family_queue_indices.graphics_queue_index != family_queue_indices.transfert_queue_index) {
        const auto indices = std::array<std::uint32_t, 2>{
          family_queue_indices.graphics_queue_index, family_queue_indices.transfert_queue_index};
...
void SwapChain::selectExtent() {
    ...
    if(surface_capabilities.currentExtent.width == std::numeric_limits<std::uint32_t>::max()) {
        ...
        m_extent.width = std::max(
          ...
          std::min(surface_capabilities.maxImageExtent.width, m_extent.width));
        m_extent.height = std::max(
          surface_capabilities.minImageExtent.height,
          std::min(surface_capabilities.maxImageExtent.height, m_extent.height));
    } else
...
void SwapChain::acquireSwapchainImages() {
    ...
    for(auto image : vk_device.getSwapchainImagesKHR(*m_swapchain)) {
        m_images.emplace_back(
          m_device.implementation().createBackedSwapchainImage(m_color_format, image));
