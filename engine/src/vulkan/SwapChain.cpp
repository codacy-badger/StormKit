...
/////////////////////////////////////
/////////////////////////////////////
void SwapChain::presentFrame(const Framebuffer &framebuffer, const Semaphore &render_finished_semaphore, const Fence &signal_fence) {
	const auto &device_impl = m_device.implementation();
	const auto &vk_device = device_impl.vkDevice();

	m_current_present_index = (m_current_present_index + 1) % MAX_IMAGE_IN_FLIGHT;

	m_current_image_index = vk_device.acquireNextImageKHR(*m_swapchain, std::numeric_limits<std::uint64_t>::max(),
														  m_image_available_semaphores[m_current_present_index].implementation().vkSemaphore(), nullptr).value;

	{
		auto &present_command_buffer = m_present_command_buffers[m_current_present_index];
		...
		present_command_buffer.implementation().transitionImageLayout(framebuffer_image.image.get(),
																	 vk::ImageLayout::eColorAttachmentOptimal,
																	 vk::ImageLayout::eTransferSrcOptimal,
																	 vk::PipelineStageFlagBits::eColorAttachmentOutput,
																	 vk::PipelineStageFlagBits::eTransfer);

		present_command_buffer.implementation().transitionImageLayout(m_swapchain_images[m_current_image_index],
																	 vk::ImageLayout::ePresentSrcKHR,
																	 vk::ImageLayout::eTransferDstOptimal,
																	 vk::PipelineStageFlagBits::eTransfer,
																	 vk::PipelineStageFlagBits::eTransfer);

		present_command_buffer.implementation().blitImage(framebuffer_image.image.get(),
		...
		present_command_buffer.submit({ &render_finished_semaphore, &m_image_available_semaphores[m_current_present_index] },
		{ &m_render_finished_semaphores[m_current_present_index] }, {PipelineStage::COLOR_ATTACHMENT_OUTPUT, PipelineStage::COLOR_ATTACHMENT_OUTPUT}, &signal_fence);
	}

	{
		const auto semaphores = std::array<vk::Semaphore, 1>{ m_render_finished_semaphores[m_current_present_index].implementation().vkSemaphore() };
...
void SwapChain::createSwapChain() {
	...
	const auto family_queue_indices = m_device.implementation().familyQueuesIndices();

	//avoid layer error
	m_device.implementation().physicalDevice().implementation().vkPhysicalDevice().getSurfaceSupportKHR(family_queue_indices.transfert_queue_index, vk_surface);
	...
	if(family_queue_indices.graphics_queue_index != family_queue_indices.transfert_queue_index) {
		const auto indices = std::array<std::uint32_t, 2>{family_queue_indices.graphics_queue_index, family_queue_indices.transfert_queue_index};
	...
	storm::DLOG("Renderer (vulkan)"_module, "Swapchain allocated at %{1}", &m_swapchain.get());
}
...
void SwapChain::selectFormat() {
	...
	if(std::size(surface_format) == 1 && surface_format[0].format == vk::Format::eUndefined)
...
void SwapChain::selectExtent() {
	...
	if(surface_capabilities.currentExtent.width == std::numeric_limits<std::uint32_t>::max()) {
		...
		m_extent.width = std::max(surface_capabilities.minImageExtent.width, std::min(surface_capabilities.maxImageExtent.width, m_extent.width));
		m_extent.height = std::max(surface_capabilities.minImageExtent.height, std::min(surface_capabilities.maxImageExtent.height, m_extent.height));
	} else
...
void SwapChain::createImageViews() {
	...
	for(const auto &image : m_swapchain_images) {
		...
		auto &image_view = m_swapchain_image_views.emplace_back(vk_device.createImageViewUnique(create_info));
		storm::DLOG("Renderer (vulkan)"_module, "ImageView allocated at %{1}", &image_view.get());
	}
}
