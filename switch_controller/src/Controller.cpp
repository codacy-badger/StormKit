...
void Controller::initDevice() {
	...
	sendSubCommand(RUMBLE_PLUS_SUBCOMMAND<std::uint8_t>,
	    VIBRATION_CONTROL<std::uint8_t>, data);
	sendSubCommand(RUMBLE_PLUS_SUBCOMMAND<std::uint8_t>,
	    IMU_DATA_CONTROL<std::uint8_t>, data);

	data.clear();
	data.emplace_back(0x31);

	sendSubCommand(RUMBLE_PLUS_SUBCOMMAND<std::uint8_t>,
	    DATA_RATE_CONTROL<std::uint8_t>, data);
...
void Controller::connect() {
	...
	m_device.connect(COMMON_VENDOR_HID<std::uint16_t>, controller_type);
}

void Controller::disconnect() { m_device.disconnect(); }

std::array<std::uint8_t, 34> Controller::getSubResponse(
	...
	do {
		...
		std::copy(
		    std::begin(rep) + 14, std::begin(rep) + 14 + 34, std::begin(buff));
...
void Controller::setLED(PlayerLed solid, PlayerLed flash) {
	std::vector<std::uint8_t> data;
	data.emplace_back(static_cast<std::uint8_t>(solid)
	                  | (static_cast<std::uint8_t>(flash) << 4));

	sendSubCommand(
	    RUMBLE_PLUS_SUBCOMMAND<std::uint8_t>, LED_CONTROL<std::uint8_t>, data);
...
void Controller::setHomeLED() {
	...
	sendSubCommand(RUMBLE_PLUS_SUBCOMMAND<std::uint8_t>,
	    HOME_LED_CONTROL<std::uint8_t>, data);
...
Controller::DeviceInfo Controller::requestDeviceInfos() {
	if (!m_device_info) {
		...
		sendSubCommand(RUMBLE_PLUS_SUBCOMMAND<std::uint8_t>,
		    DEVICE_INFO_REQUEST<std::uint8_t>, std::vector<std::uint8_t> {});
	}
...
std::pair<std::uint16_t, std::uint8_t> Controller::encodeFreq(float freq) {
	...
	std::uint8_t encoded_hex_freq = static_cast<std::uint8_t>(
	    std::round(std::log2(static_cast<double>(freq) / 10.0) * 32.0));
...
std::array<std::uint8_t, 4> Controller::encode(
    std::pair<std::uint16_t, std::uint8_t> f) {
