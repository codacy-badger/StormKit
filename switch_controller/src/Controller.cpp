...
void Controller::connect() {
    ...
    switch(m_type) {
        case ControllerType::Left: controller_type = RIGHT_PRODUCT_HID<std::uint16_t>; break;
        case ControllerType::Right: controller_type = LEFT_PRODUCT_HID<std::uint16_t>; break;
        case ControllerType::Pro: controller_type = PRO_PRODUCT_HID<std::uint16_t>; break;
    }
...
std::array<std::uint8_t, 34> Controller::getSubResponse(std::uint8_t subcommand) {
    ...
    do {
        ...
        if(rep[ 14 ] != subcommand) { m_reply_cache.emplace(rep[ 14 ], buff); }
    } while(rep[ 0 ] != 0x21);
...
Controller::DeviceInfo Controller::requestDeviceInfos() {
    if(!m_device_info) {
        ...
        sendSubCommand(
          RUMBLE_PLUS_SUBCOMMAND<std::uint8_t>,
          DEVICE_INFO_REQUEST<std::uint8_t>,
...
std::pair<std::uint16_t, std::uint8_t> Controller::encodeFreq(float freq) {
    ...
    std::uint8_t encoded_hex_freq =
      static_cast<std::uint8_t>(std::round(std::log2(static_cast<double>(freq) / 10.0) * 32.0));
