...
namespace storm::reflection {
    ...
    struct Reflection<ReturnType(Params...)> {
        static ReturnType invoke(const std::string &name, Params &&... params) {
            return ReflectableManager::instance().reflect<ReturnType, Params...>(
              name, std::forward<Params>(params)...);
        }

        static ReturnType
        invoke(const std::string &class_name, const std::string &name, Params &&... params) {
...
#define REGISTER_FUNCTION(func)                                                    \
    storm::reflection::ReflectableManager::instance().registerReflectableFunction( \
      std::make_unique<storm::reflection::Function<decltype(func)>>(#func, func));

#define REGISTER_FUNCTION_CN(func_name, func)                                      \
    storm::reflection::ReflectableManager::instance().registerReflectableFunction( \
      std::make_unique<storm::reflection::Function<decltype(func)>>(func_name, func));
