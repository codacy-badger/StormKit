...
namespace storm::entities {
	...
	void EntityManager::destroyComponent(Entity entity) {
		...
		auto it = std::find_if(std::begin(m_components[entity]), std::end(m_components[entity]), [](auto &i){
			if(i.first == T::TYPE)
				return true;
	...
	std::vector<Entity> EntityManager::entitiesWithComponent() {
		...
		std::for_each(std::begin(m_components), std::end(m_components), [&entities](const auto &pair){
