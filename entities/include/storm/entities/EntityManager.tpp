...
namespace storm::entities {
	...
	T &EntityManager::addComponent(Entity entity) {
		static_assert(
		    std::is_base_of<Component, T>::value, "T must be a Component");
		static_assert(
		    T::TYPE != Component::INVALID_TYPE, "T must have T::type defined");
	...
	void EntityManager::destroyComponent(Entity entity) {
		...
		ASSERT(hasEntity(entity), "entity param need to be existante entity");
		ASSERT(
		    hasComponent<T>(entity), "this entity don't have this component");
	...
	std::vector<Entity> EntityManager::entitiesWithComponent() {
		...
		std::for_each(std::begin(m_components), std::end(m_components),
		    [&entities](const auto &pair) {
			    ...
			    auto        it  = std::find_if(
                    std::begin(map), std::end(map), [](const auto &component) {
	...
	T &EntityManager::getComponent(Entity entity) {
		...
		ASSERT(
		    hasComponent<T>(entity), "this entity doesn't have this component");
	...
	std::vector<T *> EntityManager::componentsOfType() {
		...
		for (auto i : m_entities) {
			if (hasComponent<T>(i)) {
				vec.push_back(getComponent<T>(i));
			}
		}
