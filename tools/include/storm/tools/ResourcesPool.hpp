...
namespace storm::tools {
	...
	class ResourcesPool : public core::NonCopyable {
			struct ResourcePoolDeleter {
				explicit ResourcePoolDeleter(Key key, std::weak_ptr<ResourcesPool<Key, Resource, Deleter>*> owner)
		...
		public:
			...
			explicit ResourcesPool() : m_shared_this(new ResourcesPool<Key, Resource, Deleter>*(this)) { }
			...
			ScopedResourcePtr acquire(const Key &key) {
				...
				ScopedResourcePtr ptr {value_it->second.release(), ResourcePoolDeleter{*key_it, m_shared_this}};
				m_pool.erase(value_it);
				m_keys.erase(key_it);
