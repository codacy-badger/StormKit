...
namespace storm::tools {
	...
	public:
		...
		TreeNode::Index insert(const TreeNode &node,
		    TreeNode::Index parent_index, TreeNode::Index previous_sibling);
		TreeNode::Index insert(TreeNode &&node, TreeNode::Index parent_index,
		...
		inline const TreeNode &operator[](TreeNode::Index index) const {
			return m_tree[index];
		}
		inline TreeNode &operator[](TreeNode::Index index) {
			return m_tree[index];
		}

		inline std::size_t size() const { return m_tree.size(); }

		inline std::vector<TreeNode>::iterator begin() {
			return std::begin(m_tree);
		}
		...
		inline std::vector<TreeNode>::iterator end() {
			return std::end(m_tree);
		}
		inline const std::vector<TreeNode>::const_iterator end() const {
		...
		inline void clearDirties() {
			for (auto i : m_dirties) {
				m_tree[i].setDirtyBits(0);
			}
			m_dirties.clear();
		}
		inline const std::vector<TreeNode::Index> &dirties() const noexcept {
			return m_dirties;
		}
