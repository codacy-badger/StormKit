...
namespace storm::tools {
	class STORM_PUBLIC Tree final {
		public:
			...
			TreeNode::Index insert(const TreeNode &node, TreeNode::Index parent_index, TreeNode::Index previous_sibling);
			TreeNode::Index insert(TreeNode &&node, TreeNode::Index parent_index, TreeNode::Index previous_sibling);
			...
			inline const std::vector<TreeNode>::const_iterator begin()  const { return std::cbegin(m_tree); }
			inline const std::vector<TreeNode>::const_iterator cbegin() const { return std::cbegin(m_tree); }
			...
			inline const std::vector<TreeNode>::const_iterator cend() const { return std::cend(m_tree); }
