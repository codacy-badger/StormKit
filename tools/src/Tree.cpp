...
Tree::Tree() : m_first_free_index(0) {
    ...
    for(auto i = TreeNode::Index(0); i < m_tree.size() - 1; ++i) m_tree[ i ].setNextSibling(i + 1);
}
...
TreeNode::Index Tree::getFreeNode() {
    if(m_tree[ m_first_free_index ].nextSibling() == TreeNode::INVALID_INDEX) {
        ...
        for(auto i = first_new; i < new_size - 1; ++i) m_tree[ i ].setNextSibling(i + 1);
    }
...
////////////////////////////////////////
TreeNode::Index
Tree::insert(const TreeNode &node, TreeNode::Index parent_index, TreeNode::Index previous_sibling) {
...
////////////////////////////////////////
TreeNode::Index
Tree::insert(TreeNode &&node, TreeNode::Index parent_index, TreeNode::Index previous_sibling) {
    const auto index = getFreeNode();
...
void Tree::genDotFile(
  ...
  std::function<std::string_view(std::string_view)> colorize_node) const {
    ...
    for(auto i = 0; i < m_first_free_index; ++i) {
        if(operator[](i).firstChild() == TreeNode::INVALID_INDEX) continue;

        for(auto current = operator[](i).firstChild(); current != TreeNode::INVALID_INDEX;
