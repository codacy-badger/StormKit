...
class PyState : public State {
    ...
    void pause() override { PYBIND11_OVERLOAD_PURE(void, State, pause); }
    void resume() override { PYBIND11_OVERLOAD_PURE(void, State, resume); }
    void update([[maybe_unused]] std::uint64_t delta = 0U) override {
        PYBIND11_OVERLOAD_PURE(void, State, update, delta);
    }
    void render() override { PYBIND11_OVERLOAD_PURE(void, State, render); }
};
...
void initStateBinding(py::module &module) noexcept {
    py::class_<State, PyState, std::shared_ptr<State>>(module, "State")
      ...
      .def_property_readonly(
        "owner", [](PyState &state) -> StateManager & { return state.m_owner; })
    ...
    py::class_<StateManager>(module, "StateManager")
      ...
      .def(
        "requestPush",
        [](StateManager &manager, const State::Ptr &state) {
            state->update();
            manager.requestPush(state);
        })
      .def(
        "requestSet",
        [](StateManager &manager, const State::Ptr &state) { manager.requestSet(state); })
      ...
      .def("top", [](StateManager &manager) -> PyState::Ptr {
          return std::dynamic_pointer_cast<PyState>(manager.top().shared_from_this());
      });
}
