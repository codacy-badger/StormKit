...
namespace storm {
	namespace log {
		...
		public:
			...
			void log(Severity severity, const char *_format, Args &&... args) {
				...
				auto _text = core::format(
				    std::string(_format), std::forward<Args>(args)...);
			 ...
			 */
			template <typename... Args>
			void log(Severity severity, Module module, const char *_format,
			    Args &&... args) {
				getBase(severity, module.get());
			...
			static void installLogger(Args &&... args) {
				static_assert(
				    std::is_base_of<LogOutput, T>::value, "T must inherit Log");
				currentLogger
		...
		protected:
			...
			static void               getBase(
			                  const Severity &severity, const char *module) noexcept;
...
#ifdef DEBUG_LOGGING
		log::LogOutput::getCurrentLogger().log(
		    log::LogOutput::Severity::Debug, std::forward<Args>(args)...);
#endif
	}
	...
	inline void ILOG(Args &&... args) {
		log::LogOutput::getCurrentLogger().log(
		    log::LogOutput::Severity::Info, std::forward<Args>(args)...);
	}

	template <typename... Args>
	inline void WLOG(Args &&... args) {
		log::LogOutput::getCurrentLogger().log(
		    log::LogOutput::Severity::Warning, std::forward<Args>(args)...);
	}

	template <typename... Args>
	inline void ELOG(Args &&... args) {
		log::LogOutput::getCurrentLogger().log(
		    log::LogOutput::Severity::Error, std::forward<Args>(args)...);
	}

	template <typename... Args>
	inline void FLOG(Args &&... args) {
		log::LogOutput::getCurrentLogger().log(
		    log::LogOutput::Severity::FatalError, std::forward<Args>(args)...);
	}
	template <typename... Args>
	inline void ALOG(Args &&... args) {
		log::LogOutput::getCurrentLogger().log(
		    log::LogOutput::Severity::Nop, std::forward<Args>(args)...);
	}

	STORM_PUBLIC void InstallDefaultLogOutput();
}

constexpr storm::log::Module operator"" _module(
